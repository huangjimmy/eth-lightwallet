...
KeyStore.prototype.init = function(mnemonic, pwDerivedKey, hdPathString, salt) {
  ...
  if ( (typeof pwDerivedKey !== 'undefined') && (typeof mnemonic !== 'undefined') ){
    ...
    var priv = mnemonic;
    var isValidPri = true;
    var isValidMnemonic = true;
    if (priv && !KeyStore.isPrivateKeyValid(priv)) {
       isValidPri = false;//throw new Error('Private key does not satisfy the curve requirements (ie. it is invalid)')
    }
    if (!Mnemonic.isValid(mnemonic, Mnemonic.Words.ENGLISH)){
        isValidMnemonic = false;
    }

    if(!isValidPri && !isValidMnemonic){
        throw new Error('KeyStore: Invalid mnemonic or invalid private key');
    }
    ...
    if(isValidMnemonic) {
        ...
        this.encSeed = "";
        this.encHdRootPriv = encryptString("HDRootPriv", pwDerivedKey);
        this.computeNewAddress(pwDerivedKey, new Buffer(priv,"hex"));
    }
  }
}
...
KeyStore.prototype.isDerivedKeyCorrect = function(pwDerivedKey) {
  ...
  var paddedSeed = KeyStore._decryptString(this.encHdRootPriv, pwDerivedKey);
...
KeyStore.prototype._generatePrivKeys = function(pwDerivedKey, n) {
  ...
  if(this.encSeed == "" || !(this.encSeed.encStr)){
      return [];
  }
...
KeyStore.isPrivateKeyValid = function (privKey){
    try {
        var priv = new Buffer(privKey, 'hex');
        return ethUtil.isValidPrivate(priv)
    }catch(err){
        return false;
    }
}
...
KeyStore.prototype.getSeed = function (pwDerivedKey) {
  ...
  if(this.encSeed && this.encSeed.encStr && this.encSeed.nonce){
...
KeyStore.prototype.computeNewAddress = function(pwDerivedKey, privateKey){
    if(!this.isDerivedKeyCorrect(pwDerivedKey)) {
        throw new Error("Incorrect derived key!");
    }

    var address = KeyStore._computeAddressFromPrivKey(privateKey);
    var encPrivKey = KeyStore._encryptKey(privateKey, pwDerivedKey);
...
KeyStore.prototype.generateNewAddress = function(pwDerivedKey, n) {
  ...
  if (!this.encSeed) {
    if(this.encSeed == "" || this.encHdRootPriv == "HDRootPriv"){
        return;
     }
